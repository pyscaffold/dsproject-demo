...
# and `pip install demo-dsproject` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "demo-dsproject", "/bin/bash", "-c"]

# build package
RUN tox -e build
RUN mamba env export -n demo-dsproject -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "demo-dsproject", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "demo-dsproject", "YOUR_CONSOLE_SCRIPT"]
